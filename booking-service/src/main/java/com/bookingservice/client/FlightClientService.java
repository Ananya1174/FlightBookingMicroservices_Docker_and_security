...
public class FlightClientService {
    ...
    public FlightDto getFlightFallback(Long flightId, Throwable t) {
        log.warn("FlightClientService.getFlightFallback invoked for id={} cause={}",
                flightId, t == null ? "null" : t.toString());
        throw new ResponseStatusException(
                HttpStatus.SERVICE_UNAVAILABLE,
    ...
    public void bookSeats(Long flightId, List<SeatBookingRequest> seats) {
        flightClient.bookSeats(flightId, seats);
    }

    public void releaseSeats(Long flightId, List<String> seatNumbers) {
        flightClient.releaseSeats(flightId, seatNumbers);
    }
}