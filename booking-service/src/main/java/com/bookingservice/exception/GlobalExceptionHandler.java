...
/**
 * Centralized error responses so Postman / frontend always gets consistent JSON:
 * {
 *   "timestamp": "2025-12-08T12:00:00Z",
 *   "status": 400,
 *   "error": "Bad Request",
 *   "message": "Detailed message here",
 *   "path": "/api/flight/booking/..."
 * }
 */
@RestControllerAdvice
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {

    // Handle ResponseStatusException thrown explicitly in services/controllers
    @ExceptionHandler(ResponseStatusException.class)
    public ResponseEntity<Map<String,Object>> handleResponseStatusException(ResponseStatusException ex,
                                                                            HttpServletRequest req) {
        Map<String,Object> body = standardBody(ex.getStatusCode(), Optional.ofNullable(ex.getReason()).orElse(ex.getMessage()), req.getRequestURI());
        return new ResponseEntity<>(body, ex.getStatusCode());
    }

    // Handle JSR-303 validation errors from @Valid in controllers
    @Override
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(org.springframework.http.converter.HttpMessageNotReadableException ex,
                                                                  ...
                                                                  HttpStatusCode status,
                                                                  WebRequest request) {
        String msg = "Malformed request body: " + Optional.ofNullable(ex.getMostSpecificCause()).map(Throwable::getMessage).orElse(ex.getMessage());
        String path = extractPath(request);
    ...
    public ResponseEntity<Map<String,Object>> handleDataIntegrity(DataIntegrityViolationException ex, HttpServletRequest req) {
        ...
        Map<String,Object> body = standardBody(HttpStatusCode.valueOf(409), "Database error: " + detail, req.getRequestURI());
    ...
    public ResponseEntity<Map<String,Object>> handleOptimisticLock(Exception ex, HttpServletRequest req) {
        Map<String,Object> body = standardBody(HttpStatusCode.valueOf(409), "Concurrent update. Please retry.", req.getRequestURI());
        return ResponseEntity.status(409).body(body);
    }
    ...
    public ResponseEntity<Map<String,Object>> handleAll(Exception ex, HttpServletRequest req) {
        // log actual exception server-side (don't leak stacktrace to client)
        ex.printStackTrace();
        Map<String,Object> body = standardBody(HttpStatusCode.valueOf(500), "Internal server error", req.getRequestURI());
        return ResponseEntity.status(500).body(body);
    }

    // Helper: build consistent body
    private Map<String,Object> standardBody(HttpStatusCode statusCode, String message, String path) {
        ...
        String reason = Optional.ofNullable(HttpStatus.resolve(statusValue))
