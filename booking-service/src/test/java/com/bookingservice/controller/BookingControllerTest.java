...
class BookingControllerTest {
	...
	private BookingResponseDto sampleResponse(String pnr, Long flightId, String userEmail) {
		return BookingResponseDto.builder().pnr(pnr).flightId(flightId).userEmail(userEmail).numSeats(1)
				.totalPrice(100.0).status("ACTIVE").createdAt(Instant.now())
	...
	void bookTicket_success_returnsCreatedAndLocation() throws Exception {
		BookingRequest req = BookingRequest.builder().userEmail("user@example.com").numSeats(1)
				.passengers(List.of(PersonDto.builder().name("Alice").age(30).gender("F").seatNumber("A1").build()))
				.build();
		...
		mvc.perform(post("/api/flight/booking/{flightId}", 5L).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(req)).header("X-User-Email", "user@example.com"))
				.andExpect(status().isCreated())
				.andExpect(header().string("Location",
						org.hamcrest.Matchers.containsString("/api/flight/ticket/PNRABC12")))
				.andExpect(jsonPath("$.pnr").value("PNRABC12")).andExpect(jsonPath("$.flightId").value(5))
	...
	void bookTicket_missingHeader_returnsBadRequest() throws Exception {
		...
		when(bookingService.createBooking(any(), isNull(), anyLong()))
				.thenThrow(new org.springframework.web.server.ResponseStatusException(
						org.springframework.http.HttpStatus.BAD_REQUEST, "X-User-Email header is required"));

		mvc.perform(post("/api/flight/booking/{flightId}", 1L).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(req))).andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.message").exists());
	}

	@Test
	void bookTicket_invalidBody_returnsValidationErrorJson() throws Exception {

		BookingRequest req = BookingRequest.builder().userEmail("user@example.com").numSeats(null).passengers(List.of())
				.build();

		when(bookingService.createBooking(any(), eq("user@example.com"), anyLong()))
				.thenThrow(new org.springframework.web.server.ResponseStatusException(
						org.springframework.http.HttpStatus.BAD_REQUEST, "numSeats must be provided and > 0"));
	...
	void getByPnr_success_and_notFound() throws Exception {
		...
		mvc.perform(get("/api/flight/ticket/{pnr}", "PNR1")).andExpect(status().isOk())
				.andExpect(jsonPath("$.pnr").value("PNR1")).andExpect(jsonPath("$.userEmail").value("a@b.com"));

		when(bookingService.getByPnr("NOTFOUND")).thenThrow(new org.springframework.web.server.ResponseStatusException(
				org.springframework.http.HttpStatus.NOT_FOUND, "PNR not found"));

		mvc.perform(get("/api/flight/ticket/{pnr}", "NOTFOUND")).andExpect(status().isNotFound())
	...
	void history_returnsList() throws Exception {
		...
		mvc.perform(get("/api/flight/booking/history/{emailId}", "x@y.com")).andExpect(status().isOk())
				.andExpect(jsonPath("$[0].pnr").value("P1")).andExpect(jsonPath("$[1].pnr").value("P2"));
	}
	...
	void cancel_success_and_forbidden() throws Exception {
		...
		mvc.perform(delete("/api/flight/booking/cancel/{pnr}", "PNRXYZ").header("X-User-Email", "other@x.com"))
				.andExpect(status().isForbidden()).andExpect(jsonPath("$.status").value(403));

		when(bookingService.cancelBooking("PNR_CANCEL", "owner@x.com")).thenReturn(cancelled);

		mvc.perform(delete("/api/flight/booking/cancel/{pnr}", "PNR_CANCEL").header("X-User-Email", "owner@x.com"))
				.andExpect(status().isOk()).andExpect(jsonPath("$.pnr").value("PNR_CANCEL"))
