...
public class BookingController {
	...
	public ResponseEntity<BookingResponseDto> bookTicket(@PathVariable("flightId") Long flightId,
			@Valid @RequestBody BookingRequest request, @RequestHeader(USER_HEADER) String userEmail) {

		BookingResponseDto resp = bookingService.createBooking(request, userEmail, flightId);

		URI location = ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/flight/ticket/{pnr}")
	...
	public ResponseEntity<Map<String, Object>> cancel(@PathVariable("pnr") String pnr,
