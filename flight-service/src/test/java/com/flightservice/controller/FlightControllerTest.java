...
class FlightControllerTest {
	...
	void init() {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(controller).setMessageConverters(converter)
				.setControllerAdvice(new com.flightservice.exception.GlobalExceptionHandler()).build();
	}
	...
	void addInventory_success_returns201_and_body() throws Exception {
		...
		mockMvc.perform(post("/api/flight/airline/inventory/add").contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsString(req))).andExpect(status().isCreated())
				.andExpect(jsonPath("$.id").value(123)).andExpect(header().string("Location", "/api/flight/123"));
